...
export async function materiaH(): Promise<ScheduleData> {
  try {
    ...
    console.log('Aulas encontradas:', aulas.map(a => a.nombre)); // Debug
    ...
    horarios.forEach(horario => {
      ...
      console.log('Procesando horario:', { dia, aula: aulaNombre, horaInicio, duration }); // Debug
      ...
      if (scheduleData[dia] && scheduleData[dia][horaInicio]) {
        ...
        if (duration > 1) {
          ...
          for (let i = 1; i < duration; i++) {
            ...
            if (scheduleData[dia][nextTimeSlot] && scheduleData[dia][nextTimeSlot][aulaNombre] === null) {
    ...
    throw new Error('Error al generar los datos de horario: ' + (error as Error).message);
  } finally {
